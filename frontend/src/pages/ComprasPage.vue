<template>
  <q-page padding>
    ...
    <q-dialog v-model="detailDialog"><q-card class="panel text-white" style="width:700px;max-width:94vw"><q-card-section class="row items-center justify-between"><div class="text-h6">Compra #{{ selected?.id }}</div><q-btn flat round icon="close" v-close-popup /></q-card-section><q-separator dark/><q-card-section><div><b>Proveedor:</b> {{ selected?.proveedor }}</div><div><b>Documento:</b> {{ selected?.nro_documento || '-' }}</div><div><b>Fecha:</b> {{ formatDate(selected?.fecha) }}</div><div class="q-mt-md"><q-list separator dark><q-item v-for="i in selected?.items || selected?.productos || []" :key="i.id"><q-item-section><q-item-label>{{ i.Nombre || i.producto?.Nombre || `Producto #${i.id_producto}` }}</q-item-label><q-item-label caption>{{ i.cantidad }} × Bs {{ money(i.costo_unitario) }}</q-item-label></q-item-section><q-item-section side>Bs {{ money(i.subtotal || i.cantidad * i.costo_unitario) }}</q-item-section></q-item></q-list></div></q-card-section></q-card></q-dialog>
  </q-page>
</template>
