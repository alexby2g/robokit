  ...
  <q-page class="store-page q-pa-md q-pb-xl">
    ...
    <q-dialog v-model="cartDialog" persistent>
      <q-card class="panel text-white cart-card">
        <q-card-section class="row items-center justify-between"><div><div class="text-h6">Tu pedido</div><div class="text-caption text-muted">Revisa productos y confirma la entrega.</div></div><q-btn flat round icon="close" v-close-popup/></q-card-section>
        <q-separator dark/>
        <q-card-section>
          ...
          <div class="row q-col-gutter-md q-mt-sm">
            ...
            <div class="col-12 col-md-6"><q-select v-model="checkout.metodo_pago" :options="paymentOptions" outlined dense dark label="Método de pago *"/></div>
          ...
          <q-banner v-if="clientLogged && checkout.metodo_pago!=='Efectivo'" class="q-mt-md info-banner" rounded><template #avatar><q-icon name="payments" color="amber-4"/></template>Después de crear el pedido podrás reportar el pago y adjuntar tu comprobante desde <b>Mi cuenta</b>.</q-banner>
          <q-banner class="q-mt-md info-banner" rounded><template #avatar><q-icon name="inventory_2" color="cyan-4"/></template>Al confirmar, las unidades quedan reservadas. El stock físico se descuenta cuando el pedido se entrega.</q-banner>
        </q-card-section>
        <q-card-actions align="right" class="q-pa-md"><q-btn flat label="Seguir comprando" v-close-popup/><q-btn class="action-primary" icon="send" label="Confirmar pedido" :disable="!cart.length" :loading="sending" @click="send"/></q-card-actions>
      </q-card>
    </q-dialog>

    <q-dialog v-model="success" persistent><q-card class="panel text-white text-center q-pa-lg success-card"><q-icon name="check_circle" color="green-4" size="68px"/><div class="text-h5 q-mt-md">Pedido recibido</div><div class="text-muted q-mt-sm">Pedido #{{orderId}}</div><div class="tracking-code q-mt-md">{{trackingCode}}</div><div class="text-caption text-muted q-mt-sm">{{clientLogged?'También quedó guardado en Mi cuenta.':'Guarda este código para consultar el estado.'}}</div><div class="row q-gutter-sm justify-center q-mt-lg"><q-btn outline color="cyan-4" label="Seguir comprando" @click="success=false"/><q-btn class="action-primary" icon="local_shipping" label="Ver estado" @click="goTracking"/></div></q-card></q-dialog>
  </q-page>
</template>
